...
public class RailsListener implements Listener {
    ...
    public RailsListener(BackroomsPlugin plugin) {
        this.plugin = plugin;
    ...
    public void onChunkLoad(ChunkLoadEvent event) {
        ...
        LevelConfig ring = plugin.getBackroomsConfig().getLevelForChunk(chunk.getX(), chunk.getZ());
        if (ring == null || !ring.isEnabled() || !"level3".equals(ring.getId())) return;

        double chance = ring.getGhostCartChance();
        if (chance <= 0) return;

        BackroomsLevel level = plugin.getBackroomsConfig().getLevelInstanceById(ring.getId());
